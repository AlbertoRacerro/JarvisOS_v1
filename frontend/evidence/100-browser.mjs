...
const PRODUCT_HEAD = "0c6cbd1f09254185fc85220684a760dc26f8169b";
const ORIGIN = "http://127.0.0.1:4173";
const API = "http://127.0.0.1:8000";
const now = "2026-08-26T04:00:00Z";
...
for (const [engineName,engine] of engines) {
  ...
  const accentContrast=async()=>page.evaluate(()=>{
    ...
    const bg=parse(css.getPropertyValue("--accent-seed")); const fg=parse(css.getPropertyValue("--color-accent-on"));
  ...
  // D: appearance/accent controls + semantic isolation + readable custom extremes + local-only writes.
  ...
  for (const seed of ["#C46B2B","#5B6FD8","#FFFFFF","#000000"]) {
    await hex.fill(seed); await page.waitForTimeout(80);
    assert.equal(await page.evaluate(()=>getComputedStyle(document.documentElement).getPropertyValue("--accent-seed").trim()),seed);
    const semanticNow=await page.evaluate(()=>getComputedStyle(document.documentElement).getPropertyValue("--color-status-danger-bg")); assert.equal(semanticNow,semanticBefore,`${seed} custom accent changed danger semantics`);
    const contrast=await accentContrast(); assert.ok(contrast.ratio>=4.5,`${seed} on-accent contrast ${contrast.ratio.toFixed(2)} < 4.5`);
  }
  ...
  await page.goto(`${ORIGIN}/settings`); await page.getByRole("heading",{name:"Appearance & accent"}).waitFor(); await page.getByLabel("Custom",{exact:true}).check();
  const darkHex=page.getByLabel("HEX",{exact:true}); for (const seed of ["#FFFFFF","#000000"]) {await darkHex.fill(seed); await page.waitForTimeout(80); const contrast=await accentContrast(); assert.ok(contrast.ratio>=4.5,`dark ${seed} on-accent contrast ${contrast.ratio.toFixed(2)} < 4.5`);} await shot("D-settings-dark");
  const inverse=await page.evaluate(()=>getComputedStyle(document.documentElement).getPropertyValue("--color-text-inverse").trim()); assert.ok(inverse && inverse!=="#101713","dark technical inverse text regressed to dark foreground");
  ...
  summary.push({engine:engineName,productHead:PRODUCT_HEAD,proofs:"A-D",customExtremes:true,reducedMotion:true,compactWidth:640,mutations:mutations.length});
  await browser.close();
}
