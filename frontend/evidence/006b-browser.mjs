...
// Same-model historical load restores exact effective values as literals and marks provenance.
...
// Editing after load creates normal dirty state; cancel protects unsaved work.
...
// Dirty confirmation is revision-bound: an intervening edit makes confirmation stale.
await page.getByRole("button", { name: "Load as working configuration" }).click();
...
// Fresh confirmation switches to the run's exact model and atomically adopts its baseline.
await page.getByRole("button", { name: "Load as working configuration" }).click();
await page.getByRole("button", { name: "Replace unsaved changes" }).click();
...
// Revert after an edit returns to the loaded historical baseline.
await lengthInput.fill("31");
await page.getByRole("button", { name: "Revert field" }).first().click();
assert.equal(await lengthInput.inputValue(), "30");

// Missing optional input becomes explicit Empty rather than an invented value.
...
// Exact unit mismatch and failed execution remain inspectable but fail closed for load.
...
// Loading itself never creates/execut es a run, mutates canonical records, or dispatches providers.
assert.equal(runnerCreateCalls, 0);
assert.equal(runnerRunCalls, 0);
assert.equal(canonicalMutationCalls, 0);
assert.equal(providerCalls, 0);

// Keyboard focus and compact/effective-200% style viewport remain usable without page overflow.
