...
const patchRequestsBeforeInvalid = parameterPatchRequests;
await page.getByLabel("Minimum", { exact: true }).fill("abc");
await page.getByRole("button", { name: "Save canonical Parameter" }).click();
await page.getByRole("alert").getByText(/Minimum must be a finite number or left blank/).waitFor();
assert.equal(parameterPatchRequests, patchRequestsBeforeInvalid, "invalid nonblank numeric text must not call canonical PATCH");
