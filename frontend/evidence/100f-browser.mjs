...
const PRODUCT_HEAD = "453c1497c3ad6e5fc51995476c150df9e1ebf928";
...
async function assertPrimaryShell(page){
  ...
  const labels = (await links.allTextContents()).map(v=>v.trim()).filter(Boolean);
...
async function exercise(page,id){
  ...
  if(id==="literature"){
    assert.ok(await visibleText(page,"unavailable") || await visibleText(page,"no bounded literature"),"Literature missing truthful unavailable state");
  }
  if(id==="roadmap"){
    ...
    if(await collapse.count()){await collapse.first().click(); await page.waitForTimeout(60); const expand=page.getByRole("button",{name:/Expand/i}); if(await expand.count()) await expand.first().click();}
  ...
  if(id==="brainstorm"){
    for(const label of ["Attach","Record","Save","Reconcile","Promote"]) await assertUnavailableButton(page,label);
  }
  if(id==="repository"){
...
for(const engine of [{name:"chromium",impl:chromium},{name:"firefox",impl:firefox}]){
  ...
  try{
    for(const surface of surfaces){
      ...
      assert.ok(await page.getByRole("main").count()===1,`${surface.id} main region missing`);
