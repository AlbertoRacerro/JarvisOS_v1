...
const PRODUCT_HEAD="bf837d251f4dbe72f92453b18a90a5aad5f20539";
...
async function assertSettingsRoute(page,section,label){assert.equal(new URL(page.url()).pathname,`/settings/${section}`,`Settings ${section} route drift`);assert.equal(await page.locator(`.final-settings--${section}`).count(),1,`Settings ${section} projection missing`);const tabs=page.getByRole("navigation",{name:"Settings sections"});assert.equal(await tabs.count(),1,"Settings visible peer tabs missing");const active=tabs.getByRole("button",{name:label,exact:true});assert.equal(await active.count(),1,`Settings ${label} tab missing`);assert.equal(await active.getAttribute("aria-current"),"page",`Settings ${label} tab is not visibly current`);}
async function exercise(page,id){
 ...
 if(id==="bluecad"){assert.ok(await visibleText(page,"Jarvis"));assert.ok(await visibleText(page,"Properties"));assert.equal(await page.locator(".shell-navigator").count(),1,"BLUECAD navigator must remain visible");assert.equal(await page.locator(".bluecad-final-stage").count(),1,"BLUECAD final workstation stage missing");assert.equal(await page.locator(".bluecad-workbench__viewport").count(),1,"BLUECAD dominant viewport frame missing");const box=await page.locator(".bluecad-workbench__viewport").boundingBox();assert.ok(box&&box.width>=400&&box.height>=220,`BLUECAD viewport collapsed: ${JSON.stringify(box)}`);assert.equal(await page.locator(".bluecad-workbench__empty-viewer").count(),1,"BLUECAD truthful empty/unavailable viewport state missing without backend data");}
 ...
 if(id==="settings"){await assertSettingsRoute(page,"appearance","Appearance");const tabs=page.getByRole("navigation",{name:"Settings sections"});await tabs.getByRole("button",{name:"AI",exact:true}).click();await page.waitForTimeout(60);await assertSettingsRoute(page,"ai","AI");await page.getByRole("navigation",{name:"Settings sections"}).getByRole("button",{name:"System",exact:true}).click();await page.waitForTimeout(60);await assertSettingsRoute(page,"system","System");await page.getByRole("navigation",{name:"Settings sections"}).getByRole("button",{name:"Appearance",exact:true}).click();await page.waitForTimeout(60);await assertSettingsRoute(page,"appearance","Appearance");}
}
...
const comparison=`<!doctype html><meta charset="utf-8"><title>100f canonical comparison</title><style>body{font-family:system-ui;margin:20px;background:#f4f1e9;color:#222}section{margin:0 0 40px;padding:16px;background:white;border:1px solid #ccc}h2{margin:0 0 6px}.pair{display:grid;grid-template-columns:1fr 1fr;gap:12px;align-items:start}figure{margin:0}figcaption{font-weight:600;margin-bottom:6px}img{display:block;width:100%;height:auto;border:1px solid #999}code{font-size:12px}@media(max-width:900px){.pair{grid-template-columns:1fr}}</style><h1>100f exact-head canonical comparisons</h1><p>Product head: <code>${PRODUCT_HEAD}</code>. Raw canonical HTML is shown before later shared-shell overlays are evaluated; reviewers must apply manifest/interaction precedence for intentional supersessions such as the five-item rail and no Board peer.</p>${rows}`;
