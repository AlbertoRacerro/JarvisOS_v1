...
const ws1Implementations = [
  implementation("ws1", "model-a", "Model A", "b".repeat(64)),
...
function output(conversion, pressureDrop, pressureUnit = "bar") {
  return {
    ...
    status: "succeeded",
    outputs: {
      conversion: { value: conversion, unit: "1" },
      pressure_drop: { value: pressureDrop, unit: pressureUnit }
    }
  };
}
...
const ws1Runs = [
  run("ws1", "run-base", "Baseline 600", "model-a", input(10, 0.2, undefined), output(0.82, 0.1)),
...
await page.route("http://127.0.0.1:8000/**", async (route) => {
  ...
  if (method === "GET" && url.pathname === "/workspaces/ws1/model-implementations") {
    ws1ModelRequests += 1;
    if (ws1ModelRequests === 1) await new Promise((resolve) => setTimeout(resolve, 350));
...
// Stale workspace proof: switch before the delayed ws1 model response returns.
...
const baselineCheckbox = page.getByLabel("Baseline 600").getByRole("checkbox");
const variantCheckbox = page.getByLabel("Variant 650").getByRole("checkbox");
...
assert.equal(await page.getByText(/Model choice/i).count(), 0, "model choice must be omitted when fixture contract has no authoritative model-choice metadata");

// Existing 089 result analytics remains composed and unit-bearing.
...
// Operator-selected baseline is keyboard reachable and changes deterministic deltas.
const variantRadio = page.getByLabel("Variant 650").getByRole("radio");
await variantRadio.focus();
await variantRadio.press("Space");
await page.getByRole("columnheader", { name: /Variant 650.*Baseline/ }).waitFor();
await page.getByText("Δ -2 m", { exact: true }).waitFor();

// Different exact model rejects configuration instead of fuzzy alignment.
await variantCheckbox.uncheck();
await page.getByLabel("Other model").getByRole("checkbox").check();
await page.getByText(/one exact model version/i).first().waitFor();

// Exact input-unit mismatch is explicit; results may remain independently comparable.
await page.getByLabel("Other model").getByRole("checkbox").uncheck();
await page.getByLabel("Bad input unit").getByRole("checkbox").check();
await page.getByText(/Units do not match the current contract/i).waitFor();
await page.getByRole("heading", { name: "Recorded results" }).waitFor();

// Malformed/partial selected run is represented, never silently dropped.
await page.getByLabel("Bad input unit").getByRole("checkbox").uncheck();
await page.getByLabel("Malformed snapshot").getByRole("checkbox").check();
await page.getByText(/Run snapshot is incomplete/i).waitFor();

// Source-run navigation remains present and comparison itself has no write authority.
...
// Effective-200%-style compact viewport: no document-level horizontal overflow; local table scrolling is allowed.
await page.setViewportSize({ width: 640, height: 900 });
await page.waitForTimeout(150);
const overflow = await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
assert.ok(overflow <= 1, `unexpected page-level horizontal overflow: ${overflow}px`);
const activeTheme = await page.evaluate(() => getComputedStyle(document.documentElement).colorScheme || "");
assert.ok(typeof activeTheme === "string");
