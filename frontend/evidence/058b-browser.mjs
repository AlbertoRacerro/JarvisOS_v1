...
function implementation(workspaceId, id, label, digest) { return { id, workspace_id: workspaceId, model_spec_id: `spec-${id}`, version_label: label, implementation_artifact_id: `artifact-${id}`, status: "accepted", script_sha256: "a".repeat(64), script_path: `${id}.py`, created_at: now, input_contract_sha256: digest, input_contract: { schema_version: 1, evaluation_mode: "forward", variables } }; }
...
function input(length, flow, roughness = undefined, lengthUnit = "m") { const payload = { length: { value: length, unit: lengthUnit }, flow: { value: flow, unit: "m3/s" } }; if (roughness !== undefined) payload.roughness = { value: roughness, unit: "m" }; return payload; }
function output(conversion, pressureDrop, pressureUnit = "bar") { return { schema_version: 1, status: "succeeded", outputs: { conversion: { value: conversion, unit: "1" }, pressure_drop: { value: pressureDrop, unit: pressureUnit } } }; }
function run(workspaceId, id, label, modelVersionId, inputPayload, outputPayload, status = "succeeded") { return { id, workspace_id: workspaceId, model_version_id: modelVersionId, run_label: label, status, input_payload: inputPayload === null ? null : JSON.stringify(inputPayload), parameter_payload: null, output_payload: outputPayload === null ? null : JSON.stringify(outputPayload), started_at: now, completed_at: now, created_at: now, notes: null }; }
const ws1Runs = [
...
let canonicalMutationCalls = 0; let runnerCreateCalls = 0; let runnerRunCalls = 0; let providerCalls = 0; let ws1ModelRequests = 0;
...
assert.equal(canonicalMutationCalls, 0); assert.equal(runnerCreateCalls, 0); assert.equal(runnerRunCalls, 0); assert.equal(providerCalls, 0);
