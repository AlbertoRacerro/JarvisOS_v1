...
const PRODUCT_HEAD = "a217e8a1f029940a9de76d10059f497e51e0dbd6";
...
async function proveProjectBasis(page, engine, report) {
  ...
  await page.getByText("112 · server-owned", { exact: false }).waitFor();
  ...
  assert.ok(await page.getByText("impact-digest-browser-proof", { exact: true }).count());
...
async function proveModels(page, engine, report) {
  ...
  await page.getByText("112 · exact lifecycle read", { exact: false }).waitFor();
