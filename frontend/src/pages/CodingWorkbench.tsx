...
function RepositorySurface({ workspaceId }: Readonly<{ workspaceId: string | null }>) {
  ...
  const openFile = async (path: string) => {
    ...
    contextPreviewGeneration.current += 1;
    inspectGeneration.current += 1;
    setSelectedPath(path); setSafeUrl(null); setPreview(""); setContextBinding(null); setInspectResult(null); setProposal(null);
    ...
    if (!requestSha) { setRepositoryError("file", "missing_exact_sha"); return; }
    try {
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      if (searchGeneration.current === requestGeneration) setRepositoryError("search", errorText(cause));
    }
  };
  ...
  const inspect = async () => {
    ...
    setInspectResult(null); setRepositoryError("inspect", null);
    try {
  ...
  const suggest = async () => {
    ...
    setProposal(null); setRepositoryError("proposal", null);
    try {
...
function RuntimeSurface() {
  ...
  return <div className="final-fusion__workbench final-fusion__workbench--coding" data-testid="coding-runtime-surface">
    <Panel title="Runtime" status={loading ? "Loading" : runtimeError ? "Read error" : runtime?.observer_status ?? "Unknown"}>
      ...
      <section className="final-fusion__compare"><div className="final-fusion__version-card"><small>Running workspace</small><strong>{humanize(live.branch ?? live.head_state, "Local runtime")}</strong><p>The process is using the locally observed workspace.</p></div><div className="final-fusion__delta">→<span>{semanticDelta.relationship}</span></div><div className="final-fusion__version-card is-remote"><small>Tracked target</small><strong>{humanize(remote.requested_ref ?? CODING_TARGET_REF)}</strong><p>{humanize(runtime?.remote_status, "Remote status unknown")}</p></div></section>
      <div className="final-fusion__summary-strip"><span>Remote ahead · {semanticDelta.aheadBy ?? "Unknown"}</span><span>Remote behind · {semanticDelta.behindBy ?? "Unknown"}</span><span>{semanticDelta.partial ? "Changed files shown" : "Changed files"} · {semanticDelta.files.length}</span><span>Evidence · {semanticDelta.partial ? "Partial — file list is truncated" : humanize(semanticDelta.status)}</span></div>
      ...
      <div className="final-fusion__context-note">Primary relationship comes from the canonical server-owned runtime alignment. Remote-ahead/remote-behind counts and changed files come from the server-owned semantic delta. The browser performs no SHA ancestry or cleanliness inference.</div>
