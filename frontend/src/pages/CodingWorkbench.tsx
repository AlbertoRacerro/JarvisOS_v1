...
function RuntimeSurface() {
  ...
  return <div className="final-fusion__workbench final-fusion__workbench--coding" data-testid="coding-runtime-surface">
    <Panel title="Runtime" status={loading ? "Loading" : runtimeError ? "Read error" : runtime?.observer_status ?? "Unknown"}>
      ...
      <div className="final-fusion__summary-strip"><span>Ahead · {semanticDelta.aheadBy ?? "Unknown"}</span><span>Behind · {semanticDelta.behindBy ?? "Unknown"}</span><span>{semanticDelta.partial ? "Changed files shown" : "Changed files"} · {semanticDelta.files.length}</span><span>Evidence · {semanticDelta.partial ? "Partial — file list is truncated" : humanize(semanticDelta.status)}</span></div>
      ...
      <div className="final-fusion__context-note">Relationship and counts come directly from server-owned runtime truth: alignment is used only when compare delta is unavailable; delta relation/counts/files are consumed when present. The browser performs no SHA ancestry or cleanliness inference.</div>
