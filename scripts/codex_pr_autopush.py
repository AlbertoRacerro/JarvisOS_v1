...
def push_current_head(repo_root: Path, *, remote: str, branch: str, changed_files: list[str], allow_workflows: bool) -> str:
...
def self_test() -> None:
    ...
    assert validate_autopush_request(
        target_branch="feature", changed_files=["backend/app/x.py", "backend/tests/test_x.py", "docs/specs/022-codex-pr-autopush.md"]
    ).allowed
    ...
    assert "merge" not in {"push", "ls-remote", "diff"}
...
def main() -> None:
    ...
    if args.verify_only:
        ...
        final = push_current_head(repo_root, remote=args.remote, branch=args.branch, changed_files=changed, allow_workflows=args.allow_workflow_changes)
