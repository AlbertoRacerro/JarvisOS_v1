...
def _rewrite_contact_patch_parser() -> None:
    target = Path(sys.argv[0])
    if target.name != "apply_074_contact_fix.py":
        return
    path = Path(__file__).with_name("apply_074_contact_fix.py")
    source = path.read_text(encoding="utf-8")
    lines = source.splitlines(keepends=True)

    helper_start = next(
        index for index, line in enumerate(lines) if line.startswith("def replace_function")
    )
    constants_start = next(
        index
        for index, line in enumerate(lines)
        if line.startswith('if "CONTACT_NEIGHBORHOOD_AXIAL_MM"')
    )
...
'''
    ...
    loop_start = next(
        index for index, line in enumerate(lines) if line.startswith("loop_pattern = re.compile")
    )
