...
- This run measures Git repository retrieval only. It does not establish SQL workspace recall, scientific validity, or Hermes operational-memory integration. Canonical SQL authoritative rereads, stale/deleted refusal, bundle expansion, and typed graph behavior are covered by deterministic tests. The Hermes adapter remains conditional on the 146 runtime boundary; this run creates no Hermes transcript or memory store.
