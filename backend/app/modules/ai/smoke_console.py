...
def run_smoke_console(request: SmokeConsoleRequest) -> SmokeConsoleResponse:
    ...
    if not token_decision.allowed:
        ...
        return _blocked_response(
            ...
            model=model,
            privacy_class=policy_decision.privacy_class,
            blocked_reason=token_decision.reason
    ...
    if live_response.error:
        if live_response.external_dispatch_state == AIExternalDispatchState.not_started:
            reconcile_scaleway_smoke_reservation(
        ...
        return _blocked_response(
            ...
            error_type=_optional_string(
                live_response.error.safe_metadata.get("error_type")
            ),
        )

    reported_input = _reported_token(
        live_response.raw_provider_metadata.get("reported_input_tokens")
    )
    reported_output = _reported_token(
        live_response.raw_provider_metadata.get("reported_output_tokens")
    )
    reconcile_scaleway_smoke_reservation(
        token_decision.metadata,
    ...
    *,
    ...
    _log_console_event(
        ...
        blocked_by_token_cap=(
            token_metadata.blocked_by_token_cap if token_metadata else False
        ),
