...
await page.goto(`${app}/runs`,{waitUntil:"domcontentloaded"}); await page.getByRole("heading",{name:"Runs",exact:true}).waitFor(); const toggle=page.getByRole("button",{name:"Show analysis",exact:true}); await toggle.click(); let dock=page.locator("#shell-analysis-dock"), heading=dock.getByRole("heading",{name:"Analysis dock",exact:true}); await heading.waitFor(); ok(await heading.evaluate(e=>document.activeElement===e),"dock open focus"); await page.keyboard.press("Escape"); await toggle.waitFor(); await page.waitForFunction(()=>document.activeElement?.textContent?.trim()==="Show analysis"); ok(await toggle.evaluate(e=>document.activeElement===e),"Escape focus restoration"); await toggle.click(); dock=page.locator("#shell-analysis-dock"); heading=dock.getByRole("heading",{name:"Analysis dock",exact:true}); await heading.waitFor(); await dock.getByRole("heading",{name:"Run comparison",exact:true}).waitFor();
...
const width=await page.evaluate(()=>[document.documentElement.scrollWidth,document.documentElement.clientWidth]); ok(width[0]<=width[1]+1,`page overflow ${width}`);
await page.goto(`${app}/engineering-data`,{waitUntil:"domcontentloaded"}); await page.getByRole("heading",{name:"Engineering Data",exact:true}).waitFor(); await page.goto(`${app}/design/flowsheet`,{waitUntil:"domcontentloaded"}); await page.getByRole("heading",{name:"Dependency & provenance",exact:true}).waitFor(); ok(errors.length===0,`browser errors ${JSON.stringify(errors)}`); await browser.close(); console.log("ANALYTICS_DOCK_BROWSER_PROOF=PASS");'''
