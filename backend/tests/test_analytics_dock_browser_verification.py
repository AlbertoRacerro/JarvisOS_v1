...
await box("Run Two").uncheck(); await box("Run kPa").check(); await dock.getByText(/incompatible exact units/i).waitFor(); ok(!(await text()).includes("200 Pa"),"unit conversion must not occur"); await box("Run kPa").uncheck();
for(const label of ["Malformed","Nonnumeric","Unitless"]){await box(label).check(); await dock.getByText(/missing|unsupported|usable|invalid/i).first().waitFor(); await box(label).uncheck();}
