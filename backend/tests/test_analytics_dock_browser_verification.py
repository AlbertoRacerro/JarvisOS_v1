...
await page.goto(`${app}/runs`,{waitUntil:"domcontentloaded"}); await page.getByRole("heading",{name:"Runs",exact:true}).waitFor(); const topbar=page.locator(".shell-topbar"); await topbar.waitFor(); const controls=page.locator(".shell-topbar__controls"); const geometry=await page.evaluate(()=>{const one=e=>{const r=e.getBoundingClientRect(),s=getComputedStyle(e);return {tag:e.tagName,text:e.textContent?.trim(),rect:{x:r.x,y:r.y,width:r.width,height:r.height,top:r.top,right:r.right,bottom:r.bottom,left:r.left},display:s.display,visibility:s.visibility,opacity:s.opacity,overflow:s.overflow,overflowX:s.overflowX,overflowY:s.overflowY,position:s.position}};return {viewport:{w:innerWidth,h:innerHeight},topbar:one(document.querySelector('.shell-topbar')),controls:one(document.querySelector('.shell-topbar__controls')),buttons:[...document.querySelectorAll('.shell-topbar__controls button')].map(one),body:one(document.body),shell:one(document.querySelector('.application-shell'))}}); console.log("TOPBAR_GEOMETRY="+JSON.stringify(geometry)); const toggle=page.getByRole("button",{name:"Show analysis",exact:true}); console.log("TOGGLE_VISIBLE="+await toggle.isVisible()); await toggle.click(); const dock=page.locator("#shell-analysis-dock"), heading=dock.getByRole("heading",{name:"Analysis dock",exact:true}); await heading.waitFor(); ok(await heading.evaluate(e=>document.activeElement===e),"dock open focus"); await dock.getByRole("heading",{name:"Run comparison",exact:true}).waitFor();
