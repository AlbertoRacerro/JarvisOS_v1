...
await page.goto(`${app}/runs`,{waitUntil:"domcontentloaded"}); await page.getByRole("heading",{name:"Runs",exact:true}).waitFor(); await page.getByRole("button",{name:"Show analysis",exact:true}).click(); const dock=page.locator("#shell-analysis-dock"); await dock.getByRole("heading",{name:"Analysis dock",exact:true}).waitFor(); assert(await dock.getByRole("heading",{name:"Analysis dock",exact:true}).evaluate(el=>document.activeElement===el),"analysis heading did not receive focus"); await dock.getByRole("heading",{name:"Run comparison",exact:true}).waitFor();
const box=label=>dock.locator("label.analytics-run-row").filter({hasText:label}).locator('input[type="checkbox"]'); await box("Run One").check(); await box("Run Two").check(); await dock.getByText("Minimum",{exact:true}).waitFor(); assert((await dock.innerText()).includes("100 Pa")&&(await dock.innerText()).includes("150 Pa")&&(await dock.innerText()).includes("50 Pa"),"valid comparison summary missing");
...
await box("Run Two").check(); removed=true; await dock.getByRole("button",{name:"Refresh",exact:true}).click(); await dock.getByText("1/6",{exact:true}).waitFor(); assert(!(await dock.innerText()).includes("Run Two"),"disappeared selected run survived refresh"); removed=false; await dock.getByRole("button",{name:"Refresh",exact:true}).click(); await box("Run Two").waitFor();
