...
await page.goto(`${app}/runs`,{waitUntil:"domcontentloaded"}); await page.getByRole("heading",{name:"Runs",exact:true}).waitFor(); const toggle=page.getByRole("button",{name:"Show analysis",exact:true}); await toggle.click(); let dock=page.locator("#shell-analysis-dock"), heading=dock.getByRole("heading",{name:"Analysis dock",exact:true}); await heading.waitFor(); ok(await heading.evaluate(e=>document.activeElement===e),"dock open focus"); await page.keyboard.press("Escape"); await toggle.waitFor(); ok(await toggle.evaluate(e=>document.activeElement===e),"Escape focus restoration"); await toggle.click(); dock=page.locator("#shell-analysis-dock"); heading=dock.getByRole("heading",{name:"Analysis dock",exact:true}); await heading.waitFor(); await dock.getByRole("heading",{name:"Run comparison",exact:true}).waitFor();
