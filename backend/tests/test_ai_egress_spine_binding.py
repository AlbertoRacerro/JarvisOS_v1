...
NOW = datetime(2026, 7, 14, 20, 30, tzinfo=UTC)


def test_queued_ai_job_rejects_response_from_different_binding():
    initialize_database()
    ...
    with pytest.raises(EgressSpineStateError, match="response binding"):
        finalize_queued_ai_job(
            queued.ai_job_id,
            status="success",
            response=mismatched,
