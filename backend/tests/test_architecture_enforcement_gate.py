...
def test_ae001_accepts_exact_owner_and_rejects_aliased_connection(tmp_path: Path) -> None:
    ...
    owner.write_text("import sqlite3\n\ndef open_sqlite_connection():\n    return sqlite3.connect('ok')\n", encoding="utf-8")
    bad = tmp_path / "backend/app/feature.py"
    bad.write_text("import sqlite3 as db\n\ndef open_other():\n    return db.connect('bad')\n", encoding="utf-8")
    config = _write_config(tmp_path, [_exception("AE001", "backend/app/core/database.py::open_sqlite_connection", "core database boundary")])
...
def test_ae002_preserves_exact_debt_and_rejects_new_dispatch(tmp_path: Path) -> None:
    ...
    debt.write_text("from app.modules.ai.providers.deepseek import DeepSeekProviderAdapter\ndef run_provider_smoke(a):\n    return a.complete('x')\n", encoding="utf-8")
    bad = tmp_path / "backend/app/new_network.py"
    bad.parent.mkdir(parents=True, exist_ok=True)
    bad.write_text("import httpx as h\n\ndef send():\n    return h.post('https://example.invalid')\n", encoding="utf-8")
    config = _write_config(tmp_path, [_exception("AE002", "backend/app/modules/ai/deepseek_provider_smoke.py::run_provider_smoke", "129")])
    result = _run(tmp_path, config)
    assert result.returncode == 1
...
def test_ae003_preserves_exact_debt_and_rejects_new_canonical_sql(tmp_path: Path) -> None:
    ...
    debt.parent.mkdir(parents=True)
    debt.write_text("def create_model_spec(c):\n    c.execute('INSERT INTO model_specs (id) VALUES (?)')\n", encoding="utf-8")
    bad = tmp_path / "backend/app/modules/other/service.py"
    bad.parent.mkdir(parents=True)
    bad.write_text("def mutate(c):\n    c.execute('UPDATE requirements SET statement = ?')\n", encoding="utf-8")
    config = _write_config(tmp_path, [_exception("AE003", "backend/app/modules/modeling/service.py::create_model_spec", "127")])
    result = _run(tmp_path, config)
    assert result.returncode == 1
...
def test_diagnostics_are_stable(tmp_path: Path) -> None:
    ...
    source.write_text("import sqlite3 as s\nimport httpx as h\ndef z():\n    s.connect('x')\n    h.get('https://example.invalid')\n", encoding="utf-8")
    config = _write_config(tmp_path)
