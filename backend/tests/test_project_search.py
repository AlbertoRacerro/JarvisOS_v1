...
def _model_fixture(_workspace_id: str):
    return [
        SimpleNamespace(
            title="Reactor model",
...
def _literature_fixture(_workspace_id: str, *, offset: int, limit: int):
    assert offset == 0
    assert limit == 100
    ...
    return SimpleNamespace(items=[source])
...
def test_project_search_orders_exact_prefix_contains_and_preserves_identity(monkeypatch) -> None:
    ...
    monkeypatch.setattr(service, "list_model_dossier_index", _model_fixture)
    monkeypatch.setattr(service, "list_literature_sources", _literature_fixture)
...
def test_owner_failure_fails_whole_request(monkeypatch) -> None:
    ...
    def broken_owner(_workspace_id: str):
        raise RuntimeError("owner read unavailable")

    monkeypatch.setattr(service, "list_model_dossier_index", broken_owner)
    monkeypatch.setattr(service, "list_literature_sources", _literature_fixture)
