...
def _artifact(workspace_id: str, path: Path, *, mime_type: str = "application/pdf", payload: bytes = b"paper") -> str:
    ...
    with open_sqlite_connection() as connection:
        connection.execute(
            ...
            VALUES (?, ?, ?, ?, 'literature_source', ?, ?, 'test', 'registered', '2026-09-10T00:00:00Z', 'spec 114 fixture')
            """,
            (artifact_id, workspace_id, path.name, str(path), mime_type, "0" * 64),
        )
        connection.commit()
    return artifact_id
...
def test_source_entry_provenance_used_by_and_safe_content(client: TestClient) -> None:
    ...
    artifact_id = _artifact(
        workspace_id,
        build_paths().artifacts_dir / "literature" / "paper.pdf",
        payload=b"%PDF-1.4 safe literature",
    )
    payload = {
    ...
    assert content.content == b"%PDF-1.4 safe literature"
...
@pytest.mark.parametrize(
    ("path_kind", "mime_type"),
    [("outside", "application/pdf"), ("secrets", "application/pdf"), ("inside", "application/octet-stream")],
)
def test_unsafe_backing_content_is_never_served(client: TestClient, tmp_path: Path, path_kind: str, mime_type: str) -> None:
    workspace_id, _ = _workspace_ids(client)
