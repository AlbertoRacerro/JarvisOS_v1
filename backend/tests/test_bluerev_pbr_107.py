...
from app.modules.engineering.evaluator_contracts import EvaluationRequest, EvaluationResult, validate_evaluation_result
...
def _request(*, design: dict[str, Any] | None = None, sourced: dict[str, Any] | None = None,
             unsourced: tuple[str, ...] = (), subject: dict[str, str] = MODEL) -> EvaluationRequest:
    now = datetime.now(UTC)
    inputs = []
    for name, (value, unit) in {**FIXTURE["sourced"], **(sourced or {})}.items():
...
def test_periodic_day_night_run_conserves_nitrogen_and_reports_units() -> None:
    ...
    out = _out(result)
    assert result.fidelity == "reduced_order" and result.qualification_record_ref is None
...
@pytest.mark.parametrize(("kwargs", "expected"), [
    ({"unsourced": ("max_specific_growth_rate",)}, ("invalid_input", "parameter_provenance_missing")),
    ...
    ({"design": {"liquid_velocity": [0.02, "m/s"]}}, ("outside_validity_domain", "stack:transitional_regime")),
])
