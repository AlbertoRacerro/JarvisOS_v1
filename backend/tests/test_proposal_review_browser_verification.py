...
await nav.getByRole("button",{name:/parameter.*Tube diameter/i}).click(); await page.getByText("0.25",{exact:true}).waitFor(); await page.getByRole("button",{name:"Accept",exact:true}).click(); await nav.getByRole("button",{name:/parameter.*Tube thickness/i}).waitFor(); ok(replacementCalls===1&&genericReplacementCalls===0,"configured replacement did not use dedicated endpoint"); await page.getByText(/freshness invalidation affected 2/).waitFor();
await nav.getByRole("button",{name:/parameter.*Tube thickness/i}).click(); await page.getByRole("button",{name:"Accept",exact:true}).click(); await page.getByText("replacement freshness conflict",{exact:true}).waitFor(); ok((await nav.getByRole("button",{name:/Tube thickness/i}).count())===1,"failed replacement disappeared from canonical proposed list");
const decisionButton=nav.getByRole("button",{name:/decision.*Select tube/i}); await decisionButton.click(); await page.getByRole("button",{name:"Reject",exact:true}).click(); await nav.getByRole("button",{name:/assumption.*seawater density/i}).waitFor(); ok(rejected.some(r=>r.id==="d1"),"decision reject not persisted by canonical mock"); const focusedAfterReject=await page.evaluate(()=>document.activeElement?.getAttribute("aria-pressed")!==null||document.activeElement?.tagName==="SELECT"); ok(focusedAfterReject,"focus did not recover after selected proposal disappeared");
await filter.selectOption("accepted"); await nav.getByText(/2 records/).waitFor(); ok((await nav.innerText()).includes("Flow rate")&&(await nav.innerText()).includes("Tube diameter"),"accepted filter did not show canonical accepted records"); await filter.selectOption("rejected"); await nav.getByText(/1 record/).waitFor(); ok((await nav.innerText()).includes("Select tube"),"rejected filter did not show canonical rejected record"); await filter.selectOption("superseded"); await nav.getByText(/1 record/).waitFor(); ok((await nav.innerText()).includes("Tube diameter"),"superseded filter did not show canonical superseded record"); await filter.selectOption("all"); await nav.getByText(/5 records/).waitFor(); ok((await page.getByRole("button",{name:"Accept",exact:true}).count())===0,"historical selected record exposed transition action");
...
for(const href of ["/design/model","/design/results","/design/flowsheet","/bluecad","/runs","/engineering-data"]){ok((await page.locator(`a[href="${href}"]`).count())>0,`legacy/canonical reachability missing ${href}`)}
