...
def _analysis(tmp_path: Path) -> dict:
    ...
    return {"schema_version": "bluecad_analysis_spec_v0_1", "analysis_id": "a1", "analysis_type": "static", "geometry": {"step_path": str(step), "manifest_path": str(manifest)}, "material": {"name": "steel", "E": 200000, "nu": 0.3, "rho": 7.8e-9, "yield_strength": 250}, "bcs": [{"port_label": "run1.port_a", "kind": "fixed"}], "loads": [{"port_label": "joint1.port_b", "force": [1, 0, 0]}], "mesh": {"target_size": 5.0, "refinements": {"run1.port_a": 2.0}}, "pass_criteria": {"max_displacement": 1.0}}


def test_geo_physical_surfaces_and_fake_happy_path(tmp_path: Path) -> None:
    ...
    assert result["attempts"][0]["counts"]["physical_groups"] == {"BC_run1_port_a": 1, "LOAD_joint1_port_b": 1}
...
def test_mesh_fail_retries_once_with_half_target(tmp_path: Path) -> None:
    ...
    assert result["verdict"] == "fail"
    assert [a["target_size"] for a in result["attempts"]] == [5.0, 2.5]
