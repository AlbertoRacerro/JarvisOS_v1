...
This PR records that target policy but does not activate it by itself. Accepted
ADR-057 remains the durable authority requiring external confirmation until
ADR-059 is merged and explicitly reconciles the conflicting sentence. Consequently
059b remains blocked and no autopilot runtime implementation may begin yet.
...
The amendment now uses the same rule and does not impose a contradictory permanent
ban based only on the source label.
...
3. the amendment is target policy only until ADR-059 reconciles accepted ADR-057.
...
- 059a: `merged`, PR #90;
- 059b: `blocked` until this amendment and ADR-059 merge and the full implementation
  spec is reconciled;
...
- GitHub Actions executes on the final reconciled head;
