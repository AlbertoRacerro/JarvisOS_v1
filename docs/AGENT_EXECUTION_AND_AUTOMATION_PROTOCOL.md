...
10. degraded quorum is valid only from **durable exact-head repository-native evidence** inspectable by a later coordinator at merge time. That evidence must bind the exact head and record: the Claude and Codex request identifiers/timestamps; the two qualifying subsequent builder wake markers; each PASS-producing builder session identity and verdict; and mutation provenance sufficient to prove that at least one qualifying reviewer session contributed no material mutation to the head. PR comments/reviews or another already-canonical repo-native coordination surface may carry this evidence. Automation titles, private chat state, scheduler memory, or unverifiable model claims do not count. If any prerequisite cannot be reconstructed deterministically from durable evidence, degraded quorum is unavailable and the semantic gate remains open.
