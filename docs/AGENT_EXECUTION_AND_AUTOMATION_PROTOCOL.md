...
7. the maintainer explicitly authorizes a bounded degraded-quorum latency fallback: if the exact head is unchanged, both Claude and Codex have been requested, neither has produced a consumable verdict, and two subsequent scheduled Builder A/B/C/D wake-ups have occurred after the first current-head external request, external latency stops blocking and two separately persisted severe exact-head Builder PASS verdicts, each tied to one of those qualifying wake markers, may satisfy the degraded semantic-review gate;
...
10. degraded quorum is valid only from **durable exact-head repository-native evidence** inspectable by a later coordinator at merge time. That evidence must bind the unchanged exact head and record: the Claude and Codex request identifiers/timestamps; the two qualifying subsequent scheduled builder-wake markers; and the two separately persisted severe exact-head Builder PASS verdicts tied to those markers. PR comments/reviews or another already-canonical repo-native coordination surface may carry this evidence. This latency fallback does not claim authenticated reviewer identity or non-mutator provenance. Automation titles, private chat state, scheduler memory, or unverifiable model claims do not establish any prerequisite. If any required fact cannot be reconstructed deterministically from durable evidence, degraded quorum is unavailable and the semantic gate remains open.
