...
| `backend/tests/bluecad/test_pressure_mapping.py` | READ | Pressure-load topology battery for C3D4/S3 and C3D10/S6 meshes: proves all four tetrahedral face mappings (including midside-node identity), computes pressure area/scalar force/inward resultant evidence, and fails closed on missing/duplicate/empty groups, detached or malformed surfaces, unsupported/mixed element families/orders, non-BODY volume matches, duplicate surface topology and ambiguous non-manifold faces. |
