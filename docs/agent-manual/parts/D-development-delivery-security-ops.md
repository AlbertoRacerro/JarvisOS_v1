...
| `scripts/data_root_recovery/__init__.py` | READ | Public API surface for atomic data-root recovery. Re-exports completion/manifest constants, error/result types, canonical JSON/path-rebase/hash helpers and snapshot/verify/restore entry points; imported modules are not transitively credited by this read. |
| `scripts/data_root_recovery/cli.py` | READ | Data-root recovery CLI. Requires an explicit operation, exposes snapshot/verify/restore arguments, delegates to recovery primitives, emits deterministic sorted JSON on success, and converts `DataRootError`, OS and SQLite failures to stderr plus exit code 1. Imported recovery modules are not transitively credited. |
| `scripts/data_root_recovery/common.py` | READ | Shared fail-closed recovery primitives: validates bounded snapshot/database names and canonical relative paths; inventories only regular non-symlink files under included roots with case-fold collision detection; uses no-follow/open-time metadata checks and SHA-256 verification while copying; validates SQLite integrity/foreign keys; binds COMPLETE metadata to the manifest hash; and provides flavor-aware absolute-path rebasing/source-root detection. Backend settings imports are not transitively credited. |
| `scripts/data_root_recovery/restore.py` | READ | Relocation-safe restore implementation. Verifies the snapshot first; rejects symlink/invalid/overlapping targets; copies payload into a private partial root with size/SHA-256 rechecks; rebases documented artifact, runner and simulation paths inside a transactional SQLite update; fails if old source-root references remain; rechecks DB integrity, foreign keys, row counts, migration id and artifact hashes; then publishes with `os.replace`, retaining/restoring the previous target on publication failure. Imported recovery modules are not transitively credited. |
| `scripts/data_root_recovery/snapshot.py` | READ | Atomic snapshot creation and verification. Rejects symlink/overlapping roots, inventories the source before copying, uses SQLite backup plus data-version and post-copy inventory checks to detect concurrent mutation, hashes every payload into a canonical manifest/COMPLETE binding, verifies partial and final snapshots before/after atomic rename, and rotates only snapshots that independently verify. Imported common/backend modules are not transitively credited. |
...
| `scripts/local_phase_b_soft_review_model_probe.py` | READ | Evaluation-only local Phase-B model proposal harness. The model contract has no policy/permission/routing/retrieval/provider/tool/memory-write/runtime authority; the bounded panel is capped at eight unique cases, ambiguous references favor clarification, literal credentials are excluded from memory candidacy, and sensitive/IP engineering context may retain local-review value without granting external egress. Referenced schemas/holdouts/reports are not transitively credited. |
| `scripts/local_phase_b_soft_review_probe.py` | READ | Deterministic Phase-B fixture probe with no model/provider/network/tool/runtime calls. It derives advisory soft-review fields from corrected Phase-A results, schema-validates each row, enforces monotonicity so Phase B cannot override Phase A or recommend retrieval/provider use across hard blocks/clarification gates, always requires manual review, explicitly does not score semantic truth, and writes per-case plus summary evidence. Referenced Phase-A reports, holdout, schema and imported structured-probe module are not transitively credited. |
| `scripts/local_routing_eval.py` | READ | Manual Auto-classifier/capability-matrix routing evaluation entry point. Initializes the database because decision bundles request project context, loads the default or explicit fixture, builds an Auto decision bundle per prompt, records expected/actual capability and context plus local route class/classifier source/confidence, computes agreement, and writes timestamped JSON/Markdown evidence. Imported backend/eval modules, fixture data and generated reports are not transitively credited. |
| `scripts/local_worktree_actuator.py` | READ | Spec-141 compatibility/control wrapper over the immutable `.github` actuator core. It binds physical worktree identity to normalized common-git-dir/worktree paths, persists private host ownership claims, rejects corrupt/mismatched claims, uses stable non-blocking OS locks for writer serialization, rolls back a newly attached registry entry if ownership admission fails, and wraps acquire/release/write/stage/commit/push operations in the physical-worktree guard. Imported core/delivery policy is not transitively credited. |
| `scripts/local_worktree_ipc.py` | READ | Current-user local IPC adapter for the worktree actuator. It exposes only the fixed actuator dispatch table over AF_UNIX or Windows AF_PIPE, never TCP/process argv; bounds request/response JSON, uses a private 32-byte worker auth key, derives a fixed per-principal/worker endpoint, keeps capability admission server-owned rather than request-controlled, requires an explicit verifier for the actual Windows named-pipe boundary before accepting peers, chmods POSIX sockets to 0600, serves one request per lifecycle, and normalizes actuator/delivery refusals into bounded responses. Imported actuator/delivery modules are not transitively credited. |
| `scripts/router_policy_local_responder.py` | READ | Localhost-only Ollama responder adapter: validates literal loopback HTTP `/api/generate`, rejects credentials/query/fragment and redirects/proxies, bounds prompts/outputs, forces temperature zero, validates transport/JSON/response shape, and keeps builder construction side-effect free. |
| `scripts/router_policy_decision_probe.py` | READ | Thin compatibility re-export for the canonical backend RouterPolicy decision producer. It prepends the repository backend directory to `sys.path`, imports `app.modules.ai.routing.decision`, then republishes every non-dunder symbol and derives `__all__` from that canonical module. The imported backend decision module is not transitively credited. |

### Ledger continuity

Branch history contains earlier exact READ evidence for root files, `.github/**`, workflows, delivery/review/CI/continuation/codegen/recovery scripts, `docs/RUNBOOKS.md`, and data-root recovery files. This checkpoint adds direct fixed-master evidence for `scripts/router_policy_decision_probe.py`; its canonical backend decision implementation is not transitively credited.
