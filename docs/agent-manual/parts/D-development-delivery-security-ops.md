...
Branch history contains earlier exact READ evidence for root files, `.github/**`, workflows, delivery/review/CI/continuation/codegen/recovery scripts, `docs/RUNBOOKS.md`, and data-root recovery files. This checkpoint adds direct master-source evidence for `scripts/data_root_recovery/__init__.py`; its imported recovery modules are not newly credited by this facade read.
