...
This ledger preserves the previously committed Area-D READ evidence. The following newly reconciled exact path is additionally credited after direct source inspection at the baseline SHA:
...
The branch history before this commit contains the existing 79 exact READ rows for root files, `.github/**`, workflows, delivery/review/CI/continuation/codegen/recovery scripts, `docs/RUNBOOKS.md`, and data-root recovery files. This commit does not revoke those rows; it adds `scripts/check_lineage_overview.py` as the next actual-read path. Global audit must parse canonical branch history/current ledger evidence conservatively and must not infer unread paths from directory membership.
