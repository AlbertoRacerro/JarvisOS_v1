...
| `frontend/src/main.tsx` | READ | React bootstrap applies stored appearance/accent preferences before mounting `App` and establishes the ordered global CSS cascade used by every operator surface. |
| `frontend/src/operatorSemantics.ts` | READ | Defensive presentation-semantic helpers convert runtime/PR/provider/credential evidence into explicit human meanings while preserving unavailable, stale, failing and unknown states instead of guessing success. |
| `frontend/src/theme.ts` | READ | Visual preference authority for system/light/dark appearance and preset/custom accents; validates persisted input, tolerates unavailable storage/media APIs, computes contrast-aware foregrounds, and applies CSS variables/data attributes. |
| `frontend/src/vite-env.d.ts` | READ | Tiny TypeScript ambient declaration importing Vite client types; no product/runtime behavior or hidden registration. |
...
- `main.tsx` makes CSS import order part of effective operator behavior: semantic and final shell/header overrides load after the foundational layers, while surface-specific and responsive rules follow; later `100g-ui-repair.css` has final cascade authority among these imports.
- `operatorSemantics.ts` deliberately collapses unrecognized runtime alignment to `unknown` and withholds file/ahead/behind evidence in that case. PR summaries separately count stale checks/reviews, and provider/credential labels distinguish local, synthetic, network, environment, secure-persisted, invalid and absent states.
- Theme persistence is visual-only and best-effort. Invalid stored/custom values fall back to the microalgae preset, storage/media-query failures do not acquire application authority, and accent foreground selection is contrast-driven with a 4.5 text-contrast threshold.

## Canonical-ledger integration blocker

The connected GitHub file reader returns the large canonical `B-frontend-operator-ux.md` only in bounded/truncated ranges, while the available contents write operation replaces the complete file atomically and has no patch/append primitive. Replacing the canonical file from an incomplete fetch would destroy existing capability material. Therefore verified B rows are preserved durably here rather than performing a destructive canonical rewrite. Next action: reconstruct/materialize the complete canonical file safely, change its stale `MAPPING_STATUS: COMPLETE` to `IN_PROGRESS`, add `## EXPLICIT FILE COVERAGE LEDGER`, fold these verified rows into that table, then continue direct reads. This is a tooling/write-shape blocker to canonical consolidation, not a coverage exemption.

## Remaining coverage

Area B only. Literal completion is not yet proven. Fresh PR-head tree truth was inspected before this update. The remaining scope includes `frontend/package-lock.json`, `frontend/public/`, all still-unledgered `frontend/src/` API/components/pages/stages/styles/helpers/tests, plus canonical behavior/appearance assets under `docs/design-references/`. The canonical `B-frontend-operator-ux.md` still requires safe reconstruction and exhaustive one-row-per-file consolidation before its historical COMPLETE claim can be accepted.
