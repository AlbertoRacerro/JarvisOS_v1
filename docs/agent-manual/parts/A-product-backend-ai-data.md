...
| `backend/app/modules/ai/flow_grade_cohort_contracts.py` | READ | Cohort taxonomy/classification helpers: strict Decimal spend parsing, execution/dispatch/provider quality buckets and empirical-exclusion reasons. |
...
| `backend/app/modules/ai/flow_grade_cohort_store.py` | READ | Bounded terminal-flow SQLite evidence loader joining attempts, valid subjects/latest grade heads and revision/withdrawal/invalid counts; explicit truncation. |
