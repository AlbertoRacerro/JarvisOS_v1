...
| Fresh tree | exact denominator established at 2036 tracked files on master `240d5e0...`; exact normalized path set + four-ledger union is next. | IN PROGRESS |
...
1. Fetch each top-level subtree recursively at the fixed baseline and extract every blob path, prefixing the top-level directory; add the seven root blob paths.
2. Normalize/deduplicate to an exact 2036-path master set; fail closed if cardinality differs from 2036.
...
- **GLOBAL:** mechanically build the exact 2036-path normalized set and union four canonical ledgers.
...
- **D (#657):** canonical insertion of `scripts/check_lineage_overview.py` remains pending; then continue only from mechanically generated D orphan queue.
