...
| D | Canonical literal ledger contains 70 committed READ rows. Six further D paths were read from fresh master and have exact-path evidence, but are not credited until their rows are committed into the canonical D ledger. | IN PROGRESS |
...
- D READ: **70 committed canonical rows**; 6 additional inspected paths pending canonical-row commit.
...
- **D (#657):** `.github/browser-proof/**` is completely literalized. Remaining work includes committing the six newly read rows, then reading/classifying the rest of `scripts/**` plus D-owned governance/operations docs/config/tests/repository metadata.
