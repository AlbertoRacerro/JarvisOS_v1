...
Re-derive the post-cleanup JarvisOS implementation queue from exact current `master` after the maintainer-completed final visual/product inspection, using the final product contract and draft implementation pack as planning inputs while preserving all merged authority and eliminating overlap with existing planned specs.
...
4. only then should 101–110 and the newly frozen operator-product capabilities be promoted/reordered.
...
- planned specs 101–110 and any still-active planned rows that overlap the final product direction.
...
2. Classify every draft as one of:
   ...
   - `MERGE_INTO_EXISTING` — behavior belongs inside an existing planned 101–110 slice after explicit re-derivation;
...
4. State exactly which of 101–110 remain valid unchanged, which require re-derivation, which are reordered, and which are superseded/merged.
5. Allocate final canonical spec IDs/names/dependencies for retained new slices.
6. Update the binding order in `STATUS.md` once, preserving already merged work and one implementation front at a time.
7. Preserve frontend/backend separation: frontend visual references never authorize fake backend state; backend capability is not deleted merely because it lacks a current frontend consumer.
8. Preserve 066–068 and 080 freeze unless a separate explicit maintainer decision changes it.
9. Preserve 093/Aspen-like editable process topology exclusion unless later engineering-domain authority explicitly reopens it.
10. Record migration/compatibility obligations for removing old peer destinations while preserving direct links/backend capabilities where required.
...
- What security/auth/process boundary is required before any PTY/PowerShell terminal can be safe?
...
- Which 103–110 engineering-domain dependencies remain valid after 100a/100b and final operator-product needs?

## Deliverables

100c completion requires:

- one exact-master overlap/ownership audit;
- one canonical queue re-derivation document;
- one `STATUS.md` registry patch allocating/reordering retained slices;
...
- The terminal cannot be promoted without an explicit security/failure-mode spec covering local-only access, PTY lifecycle, path/cwd validation, command confirmation policy, secret handling and offline CI fakes.
...
Produce one exact-master authority map and binding queue that can implement the final maintainer-approved product without duplicate stores, fake frontend state or ad-hoc queue changes.
...
Il criterio è raggiungibile senza di esso? **no** — directly implementing the draft pack would create a parallel queue and could duplicate or pre-empt 101–110.
