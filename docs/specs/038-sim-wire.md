...
- One `simulation_runs` row is created per simulation invocation and its id is passed as `source_run_id` to mesh/FEM evidence. No `bluecad_attempts` or `bluecad_candidates` columns were added; the final implementation keeps the 044 mesh/FEM writer signatures unchanged and links `candidate_id`/`attempt_id` context afterward with `_link_sim_evidence_context`.
- The caller-supplied AnalysisSpec-minus-geometry validator rejects explicit `null`, non-finite numeric values, and unsafe CalculiX material identifiers before mesh/FEM execution; the schema was tightened with the same conservative material-name identifier contract.
