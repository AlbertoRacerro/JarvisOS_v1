...
- a trusted local-user / loopback-only worker or bridge;
- a registry of explicitly allowed repositories and actuator-owned worktrees;
...
- persistent worktree/commit identity that survives loss of the requesting model session;
...
- append-only audit evidence sufficient to reconstruct requesting principal/session, repository/worktree, requested capability, exact SHA before/after, admission/refusal decision, exit/result status, and bounded result digests.
...
- service startup/lifetime assumptions for Windows activation without granting self-update or arbitrary supervisor authority;
...
- all CI proof is offline against temporary local repositories/remotes, except a separately documented maintainer activation smoke if the final Windows credential path cannot be proven below the real OS layer.

## Definition exit criteria

This definition is complete when it establishes one independently removable local repository actuator owner, closes the authority boundary against generic terminal/filesystem/credential/merge/desktop expansion, records the Git-config and Windows-path execution hazards above, and leaves exact API/storage/credential/test contracts to the required full-spec and readiness stages.

No implementation, `ready` promotion, credential bootstrap, workstation installation, GitHub push, provider call, or product/runtime behavior change is authorized by this definition.