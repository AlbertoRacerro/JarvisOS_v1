...
3. 100b may now be freshly derived from the merged 100a audit artifact. A `NO_ACTION` 100b may complete docs-only; the recorded `CLEANUP` outcome must stop at a reviewable ready candidate set before any runtime mutation.
...
6. Keep 066–068 and 080 frozen; keep 062 itself blocked/deferred while allowing the operator-workstation queue to proceed without routine grading UI. Keep 100b `planned` until its fresh derivation/readiness is merged; keep 100c and 101–110 `planned` and held.
