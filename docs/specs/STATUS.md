...
1. 058d PROCESS-WORKSPACE-SCAFFOLD-0 definition PR #360 and readiness PR #361 are merged. Registry reconciliation now promotes 058d to `ready`, making exactly one bounded frontend-only 058d implementation front the next authorized runtime work; 100 VISUAL-IDENTITY-1 remains unauthorized until 058d implementation merges and is reconciled.
...
| 058 | cancelled | — | Unified workspace home layout | — | Cancelled as a monolithic implementation slice. Its product objective is redistributed across 070 UI foundation, 083, 091, and 029. |
...
| 058d | ready | — | PROCESS-WORKSPACE-SCAFFOLD-0 | 058b, 071b, 083, 089, 091, 096 | Definition PR #360 and [the 2026-08-24 readiness decision](058d-readiness-2026-08-24.md) under PR #361 are merged. Authorize exactly one bounded frontend-only implementation of canonical Process/Lineage route separation, `/design/flowsheet` replace-alias compatibility, preserved Lineage runtime, an inert canvas-first Process scaffold, single-owner Jarvis/Properties reuse and existing 089 persisted-run Analysis Dock content; no backend process topology/API/schema/store, React-owned process records, persistent drag/drop, solver/provider calls or 100 visual identity. |
