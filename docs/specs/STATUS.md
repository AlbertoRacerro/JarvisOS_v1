...
1. 058b VARIANT-COMPARISON-1 definition PR #352 and readiness PR #353 are merged. Exactly one bounded frontend-only 058b runtime implementation front is now authorized under the 2026-08-24 readiness decision; no backend route/schema/store, second comparison-state owner, unit converter, provider/solver/run side effect, canonical mutation, 058d process scaffold or visual-identity work is authorized. 058d and 100 remain unauthorized until 058b implementation merges and is registry-reconciled.
...
| 017 | merged | [#37](https://github.com/AlbertoRacerro/JarvisOS_v1/pull/37) | Autonomous three-tier review | — | Historical automatic chain; Cheap, Senior, and Expert reviews are now explicit maintainer actions with no automatic Codex or label actuation. |
...
| 058 | cancelled | — | Unified workspace home layout | — | Cancelled as a monolithic implementation slice. Its product objective is redistributed across 070, 083, 091, and 029. |
| 058b | ready | — | Workbench UX pass 2: variant comparison and design history | 006b, 071b, 083, 085, 089 | Re-derived as VARIANT-COMPARISON-1 under definition PR #352 and [the 2026-08-24 readiness decision](058b-readiness-2026-08-24.md) under PR #353. Authorize one bounded frontend-only V0 that extends the existing Analysis Dock to compare immutable successful runs using exact same-model configuration alignment plus existing 089 result comparability, with an explicit transient baseline, six-run cap, hostile-payload bounds, stale-generation protection, omission of unproven model-choice rows, and zero backend/store/canonical/run/provider side effects. |
