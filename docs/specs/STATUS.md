...
| 058 | planned | — | Unified workspace home layout | 006, 029, 037, 061 | Replace page-first navigation with the BLUECAD workbench/3D surface, persistent right-side AI entry, compact status strip with today/month token+spend counters per provider from existing status APIs, and shared design tokens; the home strip grows into a bounded 'today' pane (calendar, scheduled jobs, counters) fed by 070/071. |
...
| 070 | planned | — | SCHED-0: policy scheduler | 021, 066, 068 | Deterministic SQLite-backed schedule owned by the policy kernel with a single idempotent OS-triggered tick; launches pinned Hermes jobs (first: 069 consolidation) and internal checks with per-job budget, full ledger evidence, and no resident worker claim. |
| 071 | planned | — | CONNECTOR-CAL-0: calendar connector | 059a, 067 | Read-first MCP calendar connector: ingested entries are labeled content (never auto-public), polling via 070; creating or changing events is an outward action behind the egress confirmation-trigger path; credentials via existing secret handling. |
| 072 | planned | — | INGEST-0: document ingestion | 040, 059a, 063 | Bounded local pipeline registering files under the data root with digests and provenance, local text extraction, explicit human "public" corpus tagging with fail-closed unknown default, deterministic floors at ingest; feeds the 063 vault and 064 literature corpus without granting external eligibility. |
| 073 | planned | — | MEASUREMENT-0: as-measured records | 001, 040, 051 | Trigger-gated on a physical prototype: measurement records with units/uncertainty/provenance, deterministic as-designed vs as-measured diff reusing flowsheet stale propagation, and threshold alarms as ledger events; evidence never overwrites designed values and never actuates hardware. |
