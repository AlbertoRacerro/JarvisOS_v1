...
3. The next authorized front is a fresh definition-only re-derivation of 070 UI-FOUNDATION-1 from current `master`; no 070 implementation is authorized before a separate readiness decision.
...
| 070 | planned | — | UI-FOUNDATION-0: design tokens, appearance themes, and shared primitives | 006, 082, 094 | The 082/094 checkpoint is merged and recorded in [the 2026-08-04 Windows evidence](082-094-windows-checkpoint-2026-08-04.md). The next authorized step is a fresh UI-FOUNDATION-1 definition derived from current `master`; closed PR #132 and its retained branch are historical input, not implementation authority. |
