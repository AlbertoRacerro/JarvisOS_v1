...
3. Specs 070 UI-FOUNDATION-1, 083 APP-SHELL-1, 084 BLUECAD-READ-MODEL-1, 085 BLUECAD-WORKBENCH-2, 086 MODEL-INSPECTION-A0, 087 LINEAGE-OVERVIEW-1, 088 RUNS-WORKBENCH-1, 035 ENGINEERING-DATA-1, 089 ANALYTICS-DOCK-1, 054 PROPOSAL-REVIEW-1, 090 AI-THREADS-0, and 091 JARVIS-SIDECAR-1 are merged and reconciled; the 062 operator-design session remains pending and may remain pending while Phase 5 proceeds; 029 SETTINGS-1 is the next canonical planned slice and must be freshly re-derived from current `master`. The maintainer-approved workstation reference continues to guide local information hierarchy, while global visual identity remains a separate independently removable lane.
