...
| 045 | cancelled | — | Runner hardening boundary | 043 | Retired as a standalone isolation program for the current single-user loopback product. The bounded bundled-only runner cleanup is delivered without a numbered spec. Reopen only on non-loopback access, a second user, remote-agent or Hermes/MCP reachability, or a demonstrated need to execute non-bundled code. |
...
| 075 | planned | — | PROCESS-KERNEL-1: streams, components, ports, and unit operations | 043, 047, 048, 071 | Re-express bundled 047 as a typed acyclic flowsheet with dimensional boundary conversion, components, streams, scalar/material ports, reusable blocks, deterministic execution, and bit-identical canonical outputs; no recycle solver, property package, mutable flowsheet store, or new runner kind. |

## Superseded planning aliases and resolved collisions

- Historical `045 = AGENT-ORCH` references are superseded by `060`. Spec 045 is cancelled for the current single-user loopback product and may be reopened only by its explicit isolation triggers; the number must not be reused.
