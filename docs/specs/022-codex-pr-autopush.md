...
## 2026-09-13 bounded Codex result authority amendment

The maintainer explicitly authorized the deferred automatic materialization gap in this spec. The exact authority contract is `docs/specs/022-128-codex-result-authority-2026-09-13.md`. Codex may now cause a result payload to be considered automatically only through the exact `codex-result-delivery.yml` issue-comment dispatcher defined there; the dispatcher itself has no contents-write or merge authority and delegates all patch admission, validation, exact-head CAS and guarded branch mutation to the existing trusted cloud-delivery bridge. A task-local commit remains non-materialized evidence until the remote PR branch advances.
