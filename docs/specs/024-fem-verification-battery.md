...
Lamé blocked, and later declared it unblocked. A fresh code audit found a second,
...
- `mesh_adapter.py` invokes Gmsh without `-order 2`, so current volume elements
  are first order;
- the mesh adapter identifies named surfaces through bounding boxes and Gmsh
  exports boundary elements for those groups;
...
8. Existing order-1 tests and the complete backend suite remain green.
...
- use mm, N, kg, s, and MPa with no hidden unit conversion;
...
The loads remain safely in the linear-elastic range.
...
- prescribed fine target size no larger than `h / 3`;
...
- length `L = 160 mm`;
- one end fixed only to remove rigid motion;
...
- target size no larger than `(b - a) / 4` through the wall;
...
- require mean sampled radius within `1%` of `a` and mean z within one local mesh
  spacing of `L / 2`;
...
- gross-section nominal stress `sigma_nom = F / (W t) = 10 MPa`;
- finite-width gross-section correction:
  `Kt(x) = 3 - 3.14 x + 3.667 x^2 - 1.527 x^3`;
- at `x = 0.2`, `Kt = 2.506464` and
  `sigma_peak = 25.06464 MPa`;
- local target size at the hole no larger than `d / 12`;
- relative-error tolerance: `7%`.

The implementation report must cite the exact Peterson/Pilkey source edition
used for the polynomial and confirm that the formula uses gross-section nominal
stress. If that source check contradicts the equation or convention above, STOP
and amend the definition rather than silently changing the denominator.
...
4. the battery report and raw bounded artifacts are uploaded from the same run;
5. the complete backend suite and Ruff pass;
6. no conformance test is weakened, skipped by default when strict mode is
   requested, or replaced by fake output;
7. no UI, AI routing, provider behavior, promotion policy, or production default
   is changed.
...
- a prescribed tolerance requires silent solver tuning, manual mesh editing, or
  a load/BC substitution outside the documented adapter contract.
