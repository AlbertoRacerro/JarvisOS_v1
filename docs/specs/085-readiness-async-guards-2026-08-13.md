...
This addendum is normative for the 085 readiness decision in `085-readiness-2026-08-13.md` and tightens, without broadening, the frozen request-race and GLB lifecycle contracts.
...
These cases are beta-blocking acceptance criteria. A conforming 085 implementation cannot claim readiness completion or merge while either secondary asynchronous path can overwrite state belonging to a newer selection/artifact. No backend, schema, dependency, workflow, provider, credential, budget, egress or global visual-identity change is authorized by this addendum.