...
| `scripts/router_policy_local_responder.py::_stdlib_json_post_client` | localhost-only Ollama transport | exact AE002 exception only after this transport itself validates the initial endpoint as loopback and prevents redirect escape |
...
The router-policy flow validates its configured endpoint before invoking `_stdlib_json_post_client`, but that does not make the exact-exempted transport function safe in isolation: `_stdlib_json_post_client(endpoint, ...)` is directly importable and currently accepts any endpoint. Before its exact AE002 exception is valid, `_stdlib_json_post_client` itself must apply the existing strict `_validate_endpoint(endpoint)` contract before request construction/dispatch. This is deliberate validation duplication at the security boundary, not new policy: it reuses the already-existing validator and closes direct-call bypass of the exact-exempted symbol.

Both localhost clients also use standard `urllib.request` behavior, which may automatically follow an HTTP redirect from a validated loopback URL to an external `Location`. Initial-URL validation alone therefore does not make either exact exception truthful. Before either localhost-tool exception is valid, the implementation must deterministically reject redirects before following them, or enforce an equivalently fail-closed per-hop loopback check. The minimum authorized design is redirect rejection at the two existing urllib transport boundaries; no generic redirect/policy framework is authorized.
...
6. Constrain `scripts/router_policy_local_responder.py::_stdlib_json_post_client` at its own callable boundary before its retained-owner exception applies: invoke the existing `_validate_endpoint(endpoint)` before request construction/dispatch so a direct call cannot bypass the current strict localhost/http/path/no-credentials/no-query/no-fragment contract.
7. Prevent redirect escape in both retained localhost urllib transports: `scripts/local_model_structured_output_probe.py::call_ollama_chat` and `scripts/router_policy_local_responder.py::_stdlib_json_post_client`. A 3xx response from the validated loopback destination must not be automatically followed to any `Location`; reject the redirect before a second request is dispatched, or use an equivalently fail-closed per-hop loopback validation. Keep the router responder's existing endpoint contract unchanged.
8. Unknown or unresolvable dynamic calls are not to be guessed into semantic authority. The implementation should cover the frozen concrete patterns above and document residual dynamic-dispatch risk rather than invent a generic taint engine.
9. No source-text/regex-only substitute for AST ownership classification.
...
- `scripts/router_policy_local_responder.py` only for invoking its existing `_validate_endpoint` at `_stdlib_json_post_client` and bounded redirect rejection there; preserve the existing endpoint contract and higher-level responder semantics.
...
| router transport boundary | direct `_stdlib_json_post_client` call with representative external/malformed endpoint is rejected by the existing endpoint policy before request construction/dispatch; valid current localhost endpoint remains accepted up to the mocked transport seam |
...
- `_stdlib_json_post_client` is exact-exempted while remaining directly callable with an external endpoint because validation exists only in its current higher-level caller;
...
The gap is inside one existing static scanner and one focused deterministic test surface. Extending those owners with bounded call-target/binding recognition, preserving provider-import detection, adding ten exact pre-existing owner dispositions, keeping redirect-safe urllib opener dispatch visible to AE002, and making the two exact-exempted localhost urllib transports fail closed against arbitrary direct destination/redirect escape is the smallest corrective action that closes the named bypasses without turning current sanctioned control/local tooling into surprise gate debt. A new policy service, runtime instrumentation, dependency graph, directory-wide allowlist, broad redirect framework, or broad linter framework would be disproportionate and is not authorized.
