...
6. The maintainer explicitly authorizes a bounded degraded-quorum latency fallback. If the exact head is unchanged, both Claude and Codex have been requested, neither has produced a consumable verdict, and two subsequent scheduled Builder A/B/C/D wake-ups have occurred after the first current-head external request, external latency ceases to block. Two separately persisted severe exact-head Builder PASS verdicts, each tied to one of those qualifying wake markers, may then satisfy degraded quorum. A single builder verdict never self-certifies; both persisted verdicts are mandatory. Durable exact-head repository-native evidence must record the unchanged head, the Claude and Codex request identifiers/timestamps, the two qualifying subsequent scheduled builder-wake markers, and the two PASS verdicts. This latency fallback does not claim authenticated reviewer identity or non-mutator provenance; if any required fact cannot be reconstructed deterministically, degraded quorum is unavailable.
7. Any reviewer disagreement, P0/P1, blocking P2, or failed acceptance criterion blocks degraded quorum. Where practical, an independent builder forms its initial verdict before reading another builder verdict to reduce anchoring.
