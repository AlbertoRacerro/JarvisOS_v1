...
For a PR whose deterministic changed-path classifier proves that **every changed repository path is Markdown (`*.md`)**, CI may use a governance-only terminal fast path and skip dependency installation, backend runtime tests/lint, frontend build, and BLUECAD runtime canaries. The fast path must still run the deterministic registry/spec gate and repository-development governance/anti-authority self-tests relevant to Markdown authority. It fails closed to normal CI for an empty/unknown diff, classifier failure, or any changed non-Markdown path.

This docs-only fast path is evidence economy, not a correctness waiver: no executable, workflow, configuration, schema, fixture, or product/runtime file changed, while the executable tree is identical to the already validated base. Any non-Markdown change receives normal required terminal gates.
