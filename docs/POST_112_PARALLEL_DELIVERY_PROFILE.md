...
6. If the exact head is unchanged, both Claude and Codex have been requested, neither has produced a consumable verdict, and two subsequent scheduled Builder A/B/C/D wake-ups have occurred after the first current-head external request, external latency ceases to block. Two PASS verdicts from two distinct builder sessions may then satisfy degraded quorum only if at least one PASS comes from a session that performed **no material mutation contributing to that exact head**. A single builder never self-certifies.
