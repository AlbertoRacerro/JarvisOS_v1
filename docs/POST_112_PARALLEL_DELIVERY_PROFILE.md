...
If a reviewer reports a P0/P1 without evidence of that bounded family sweep, request exactly one same-head follow-up limited to the same causal/failure family before mutating, unless immediate safety urgency requires repair. Do not recursively expand review scope. When safely possible, repair the consolidated same-family P0/P1 set in one bounded mutation with minimum causal sibling tests. P2/P3 remain PARK unless separately elevated and do not delay an otherwise valid merge. A head mutation invalidates affected review evidence and restarts exact-head review requirements as usual.
