...
For other pull requests, CI may classify only conservative macro-domains and run the affected domain gates in parallel. Frontend-only changes may skip backend and BLUECAD runtime lanes. Backend changes run backend/static and backend runtime coverage plus BLUECAD coverage conservatively; while frontend contracts remain manually mirrored, the cheap frontend build also remains a regression gate for backend changes. Changes to workflow/control infrastructure, scripts, root configuration, dependency/control files, unknown paths, or any path outside the explicitly classifiable domain trees fail closed to full CI rather than relying on source-file-to-test inference.

Parallelization and sharding are scheduling optimizations only. On every full path, the union of backend core shards, ordinary BLUECAD shards, and explicitly canary-owned BLUECAD tests must preserve the complete previously required pytest coverage. A test deliberately owned by a dedicated canary is not repeated in ordinary shards; no other test may be dropped or multiply owned merely to shorten wall-clock. Independent expensive groups may execute concurrently, but workflow green is invalid evidence if shard ownership or collection loses required coverage.

Pushes to `master` always run full CI across all domains, including after a docs-only or affected-domain merge. The PR shortcuts therefore remove redundant pre-merge work without eliminating the repository-wide regression pass on the resulting master commit.

This docs-only fast path is evidence economy, not a correctness waiver: no executable, workflow, configuration, schema, fixture, or product/runtime file changed on the pull request, while the executable tree is identical to the already validated base. A non-doc PR receives every gate selected by the conservative domain classifier, and shared/control/unknown changes receive the full path.
