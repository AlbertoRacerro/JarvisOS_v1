...
REF-041 whole-engine qualification (2026-09-24): `docs/specs/103-dwsim-whole-engine-qualification-2026-09-24.md`. Pinned DWSIM 10.2.9 ran headless WSL2 steady/recycle/tank/PID/custom-unit cases, a Windows desktop edit/save → headless reload/solve round trip, a headless edit of that desktop-saved case, and a persisted dynamic event/state replay across separate MCP processes. Input read-back and balance checks exposed MCP envelope and metadata inconsistencies, so an adapter must validate native state rather than trust call success alone. DWSIM is the preferred complete generic process-engine hypothesis, with Jarvis retaining canonical control/evidence/study/frontend authority and BlueRev supplying the qualified domain-specific delta. Exact BlueRev models, scientific comparison, Windows-native solver execution, valid pressure-flow/control proof and full connector/property projection remain unproven. Internal commercial use without software distribution is the current assumption; distribution requires a separate future review. REF-039 BioSTEAM remains an alternative engine and TEA/LCA specialist. This evidence does not by itself promote 107 ownership.
